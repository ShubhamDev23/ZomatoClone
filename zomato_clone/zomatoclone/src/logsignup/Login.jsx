...
import { auth ,signwithgoogle} from "../backend";
...
const Login = () => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      await signInWithPopup(auth,signwithgoogle);
        <Link to="/"></Link>
    } catch (error) {
      console.error(error);
    }
  ...
  return (
    <div className="login-container">
      ...
      <div className='sub-container'>
        ...
        <h2 className='log-up-heading'>Log in</h2>
       <Link to="/"> <img 
        className='logup-logo'
        alt='cross'
        src='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQgLTcaiC2mCNtOHI_geAKMDAsGanAHe48ELA&usqp=CAU'/>
      </Link>
       </div>
      <input 
      ...
      />
      <button className='create-btn' onClick={handleLogin}>Log in</button>
      ...
      <button className='google-btn' onClick={handleGoogleSignIn}>
        ...
        <div className='lastline'>Don,t have an account?
        <Link className='linklast' to="/signup">Signup</Link>
        </div>
      </div>
